...
TimingWheel_t CreateTimingWheel(unsigned long precision,unsigned long max)
{
	unsigned long slot_size = max/precision;
...
inline static void Add(TimingWheel_t t,unsigned long slot,WheelItem_t item)
{
	if(t->slot[slot])
	{
...
int  RegisterTimer(TimingWheel_t t,WheelItem_t item,unsigned long timeout)
{
...
//激活slot中的所有事件
static void Active(TimingWheel_t t,unsigned long slot)
{
...
int UpdateWheel(TimingWheel_t t,unsigned long now)
{
	unsigned long interval = now - t->last_update;
